...
module.exports = {
  ...
  fetch: (params) => {
    ...
    return Prices
      .findOne(_.pick(params, _.keys(Prices.schema.paths)))
      .populate(populate);
  },

  /**
   * Promise to count prices.
   *
   * @return {Promise}
   */
  ...
  add: async (values) => {
    ...
    const entry = await Prices.create(data);

    // Create relational data and return the entry.
    return Prices.updateRelations({ _id: entry.id, values: relations });
  },
  ...
  edit: async (params, values) => {
    // Extract values related to relational data.
    const relations = _.pick(values, Prices.associations.map(a => a.alias));
    const data = _.omit(values, Prices.associations.map(a => a.alias));

    // Update entry with no-relational data.
    const entry = await Prices.updateOne(params, data, { multi: true });
  ...
  remove: async params => {
    ...
    await Promise.all(
      Prices.associations.map(async association => {
  ...
  search: async (params) => {
    ...
    const $or = Object.keys(Prices.attributes).reduce((acc, curr) => {
      switch (Prices.attributes[curr].type) {
        case 'integer':
        case 'float':
        case 'decimal':
